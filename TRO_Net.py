...
class Bottleneck(nn.Module):
    ...
    def __init__(self, stride=1):
        ...
        self.conv1 = nn.Conv2d(3, 8, kernel_size=3, padding=1, stride=stride)
        ...
        if stride != 1 or 3 != self.expansion * 8:
            self.shortcut = nn.Sequential(
                nn.Conv2d(3, self.expansion * 8, kernel_size=1, stride=stride),
...
class TRO_Net(nn.Module):
    def __init__(self, num_res=8):
        super(TRO_Net, self).__init__()
    ...
    def forward(self, x, y, w):
        x_z, x_, x_2, x_4, x_res1, x_res2 = self.Feature_extraction[0](x, w)
        y_z, y_, y_2, y_4, y_res1, y_res2 = self.Feature_extraction[1](y, x)
        ...
        double = self.Convs[3](double)
        double1 = double * w
        # double1 = self.Convs[2](double1)
        double = self.Decoder[0](double1)
